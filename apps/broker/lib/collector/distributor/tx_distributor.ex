defmodule Broker.Collector.TxDistributor do
  @moduledoc """

  Documentation for Broker.Collector.TxDistributor.
  ...
  @tx_feeders_num length(Application.get_env(:broker, :__TOPICS__)[:tx_trytes])
  ...
  def start_link(args) do
    GenStage.start_link(__MODULE__, args, name: :tx_distributor)
  end
  ...
  def handle_subscribe(:consumer, _, _from, queue) do
    Logger.info("TxDistributor: #{Process.get(:name)} got subscribed_to TxValidator")
    {:manual, queue}
  end

  @spec handle_subscribe(atom, tuple | list, tuple, map) :: tuple
  def handle_subscribe(:producer, _, _from, queue) do
    Logger.info("TxDistributor: #{Process.get(:name)} got subscribed_to TxFeeder")
    {:automatic, queue}
  end
