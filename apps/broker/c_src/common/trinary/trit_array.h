...
static inline trit_array_t trit_array_null() {
  trit_array_t trit_array = {.trits = NULL, .num_trits = 0, .num_bytes = 0, .dynamic = 0};
  return trit_array;
}
...
static inline trit_t trit_array_at(trit_array_p const trit_array, size_t const index) {
...
static inline uint8_t trit_array_set_at(trit_array_p const trit_array, size_t const index, trit_t const trit) {
  return flex_trits_set_at(trit_array->trits, trit_array->num_trits, index, trit);
}
...
static inline bool trit_array_is_equal(trit_array_p const lhs, trit_array_p const rhs) {
  return (lhs->num_trits == rhs->num_trits && lhs->num_bytes == rhs->num_bytes &&
...
trit_array_p trit_array_slice(trit_array_p const trit_array, trit_array_p const to_trit_array, size_t const start,
...
#define TRIT_ARRAY_DECLARE(NAME, NUM_TRITS)                        \
  ...
  struct _trit_array NAME = {(flex_trit_t *)&NAME##_trits, NUM_TRITS, NAME##_num_bytes, 0};
...
#define TRIT_ARRAY_MAKE_FROM_TRYTES(NAME, NUM_TRYTES, TRYTES)                           \
  ...
  flex_trit_t NAME##_trits[NAME##_num_bytes];                                           \
  flex_trits_from_trytes(NAME##_trits, NUM_TRYTES * 3, TRYTES, NUM_TRYTES, NUM_TRYTES); \
  struct _trit_array NAME = {(flex_trit_t *)NAME##_trits, NUM_TRYTES * 3, NAME##_num_bytes, 0};
