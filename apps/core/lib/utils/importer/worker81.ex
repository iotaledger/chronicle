defmodule Core.Utils.Importer.Worker81 do

  use GenServer
  use OverDB.Worker,
    executor: Core.Executor
  alias Core.Utils.Importer.Helper81
  require Logger
  @doc """
  ...
  def init({:zero_value, bundle}) do
    ...
    state = Map.put(Helper81.queries(true, bundle)
  ...
  def init({:value, bundle}) do
    ...
    Process.send_after(self(), :check, 10000)
    state = Map.put(Helper81.queries(false, bundle)
    |> Enum.into(%{}), :bundle, bundle)
    {:ok, state}
  end
